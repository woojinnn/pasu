...
import { defaultsFor, validateParams, type ParamsSchema } from '../params-validator';
...
describe('validateParams', () => {
  ...
  it('rejects malformed addresses', () => {
    ...
    expect(() => validateParams(schema, { addr: '0xtoo-short' })).toThrow(/address/);
  });
});

describe('defaultsFor', () => {
  it('uses schema defaults', () => {
    const schema: ParamsSchema = { cap: { type: 'integer', min: 0, max: 100, default: 25 } };
  ...
  it('applies overrides on top of defaults', () => {
    const schema: ParamsSchema = {
      ...
      label: { type: 'string', maxLen: 16, allowedChars: 'A-Za-z', default: 'demo' },
    };
    expect(defaultsFor(schema, { cap: 50 })).toEqual({ cap: 50, label: 'demo' });
  });
