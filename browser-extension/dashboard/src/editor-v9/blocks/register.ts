...
/** Built lazily (not at module import) so block labels/tooltips resolve
 *  through i18n with the language active when registration runs. */
function allBlockJson(): object[] {
  return [
    POLICY_BLOCK_JSON(),
    // scope
    SCOPE_BLOCK_JSON(),
...
export function registerBlocks(): void {
  ...
  Blockly.defineBlocksWithJsonArray(allBlockJson());
  registered = true;
}
