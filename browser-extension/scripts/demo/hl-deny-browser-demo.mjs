#!/usr/bin/env node
/**
 * Automated browser proof that the Scopeball extension BLOCKS a Hyperliquid
 ...
 *   - SHORT (b=false) → window.__scopeball_last_verdict__.allowed === false   (blocked)
 *   - LONG  (b=true)  → window.__scopeball_last_verdict__.allowed === true    (allowed)
 *
 * Prereqs:
...
const orderExpr = (isBuy) => `(async () => {
  ...
  return { outcome, verdict: window.__scopeball_last_verdict__ };
})()`;
...
async function main() {
  ...
  const installed = await evalExpr(
    "!!window[Symbol.for('__scopeball_fetch_hook_install_state__')]",
  );
  if (!installed) {
    console.error(
