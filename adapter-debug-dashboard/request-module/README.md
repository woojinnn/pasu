...
```bash
WEB_SERVER_ADDR=127.0.0.1:8080 cargo run -p adapter-debug-dashboard
```
...
```bash
cd adapter-debug-dashboard/frontend
npm install   # 처음 한 번
npm run dev
```
...
3. `adapter-debug-dashboard/request-module/userscript/scopeball.user.js` 내용 복사·붙여넣기 → 저장
...
# 터미널 2
cd adapter-debug-dashboard/frontend && npm run dev
...
│   ├── policy-engine/
│   ├── adapters/...
│   └── adapters/abi-resolver/              ★ Sourcify 디코더 + SQLite 빌더
...
│       │   └── curate_bundle.sh
│       └── src/                            (Rust 라이브러리)
└── adapter-debug-dashboard/                ★ HTTP API + React 프론트 + 브라우저 hook
    ├── src/main.rs                         (axum)
...
- `adapter-debug-dashboard/request-module/extension/` 추가
