//! UR opcode `V3_SWAP_EXACT_IN` → `Action::Swap` (V3 exact-in).
//!
//! Signature: `(address recipient, uint256 amountIn, uint256 amountOutMin,
//! bytes path, bool payerIsUser)`.
//!
//! V3 fee tiers vary per pool — `parse_v3_path` extracts the first hop's
//! fee from the packed path encoding and surfaces it as `fee_bps`.

use std::sync::Arc;

use abi_resolver::ids::UR_V3_SWAP_EXACT_IN_DECODER_ID;
use abi_resolver::{DecodedCall, DecoderId};
use policy_engine::action::common::AmountKind;
use policy_engine::action::dex::{SwapAction, SwapMode};
use policy_engine::action::envelope::{Action, ActionEnvelope, Category};

use crate::mapper::{MapContext, Mapper, MapperError, MapperId, MapperMatchKey};

use super::common::swap_amount_constraint;
use super::common::{
    asset_with_amount, decimal_from_uint, find_address, find_bytes, find_uint, map_recipient,
    parse_v3_path, token_asset_ref,
};

pub const UR_V3_SWAP_EXACT_IN_MAPPER_ID: &str = "uniswap-ur/V3_SWAP_EXACT_IN";

#[derive(Debug, Clone, Copy, Default)]
pub struct UrV3SwapExactInMapper;

impl UrV3SwapExactInMapper {
    #[must_use]
    pub const fn new() -> Self {
        Self
    }
}

impl Mapper for UrV3SwapExactInMapper {
    fn id(&self) -> MapperId {
        MapperId::new(UR_V3_SWAP_EXACT_IN_MAPPER_ID)
    }

    fn accepts(&self, decoded: &DecodedCall) -> bool {
        decoded.decoder_id.as_str() == UR_V3_SWAP_EXACT_IN_DECODER_ID
    }

    fn map(
        &self,
        ctx: &MapContext<'_>,
        decoded: &DecodedCall,
    ) -> Result<Vec<ActionEnvelope>, MapperError> {
        let recipient = map_recipient(ctx, find_address(decoded, "recipient")?);
        let amount_in = decimal_from_uint(find_uint(decoded, "amountIn")?);
        let amount_out_min = decimal_from_uint(find_uint(decoded, "amountOutMin")?);
        let path_bytes = find_bytes(decoded, "path")?;
        let path = parse_v3_path(&path_bytes)?;

        let action = SwapAction {
            swap_mode: SwapMode::ExactIn,
            input_token: asset_with_amount(
                token_asset_ref(&path.token_in),
                swap_amount_constraint(AmountKind::Exact, amount_in),
            ),
            output_token: asset_with_amount(
                token_asset_ref(&path.token_out),
                swap_amount_constraint(AmountKind::Min, amount_out_min),
            ),
            recipient,
            validity: None,
            fee_bps: path.fee_bps,
        };

        Ok(vec![ActionEnvelope {
            category: Category::Dex,
            action: Action::Swap(action),
        }])
    }
}

#[must_use]
pub fn v3_swap_exact_in_mapper_key() -> MapperMatchKey {
    MapperMatchKey {
        decoder_id: DecoderId::new(UR_V3_SWAP_EXACT_IN_DECODER_ID),
    }
}

#[must_use]
pub fn v3_swap_exact_in_mapper_arc() -> Arc<dyn Mapper> {
    Arc::new(UrV3SwapExactInMapper::new())
}
