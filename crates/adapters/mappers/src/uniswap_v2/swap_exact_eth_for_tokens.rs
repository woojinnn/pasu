//! V2 Router02 `swapExactETHForTokens` (payable) — selector 0x7ff36ab5.
//! `amountIn` comes from `msg.value`, not calldata.
...
pub const SELECTOR: [u8; 4] = [0x7f, 0xf3, 0x6a, 0xb5];
...
pub fn map(
    ...
    Ok(vec![envelope_swap(SwapAction {
        ...
        token_in,
        token_out,
        amount_in: AmountConstraint::exact(tx_value_u256(tx).to_string()),
