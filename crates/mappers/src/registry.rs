...
fn is_universal_router(to_lc: &str) -> bool {
    UR_ADDRESSES_LC.iter().any(|a| *a == to_lc)
}

pub fn protocol_for(to_lc: &str) -> Option<ProtocolRef> {
    if to_lc == V2_ROUTER_LC {
        Some(ProtocolRef { name: "uniswap".into(), version: Some("v2".into()), component: Some("router".into()) })
    } else if to_lc == V3_ROUTER_LC {
        Some(ProtocolRef { name: "uniswap".into(), version: Some("v3".into()), component: Some("router".into()) })
    } else if to_lc == SR02_LC {
        Some(ProtocolRef { name: "uniswap".into(), version: Some("swap-router-02".into()), component: Some("router".into()) })
    } else if is_universal_router(to_lc) {
        Some(ProtocolRef { name: "uniswap".into(), version: Some("universal-router".into()), component: Some("router".into()) })
    } else {
        None
    }
}
...
pub fn dispatch(
    ...
    if tx.input.len() < 4 {
        return Err(MapError::TooShort { need: 4, got: tx.input.len() });
    }
    let sel = [tx.input[0], tx.input[1], tx.input[2], tx.input[3]];
    let to_lc = tx.to.to_lowercase();

    if to_lc == V2_ROUTER_LC {
        if sel == swap_exact_tokens_for_tokens::SELECTOR { return swap_exact_tokens_for_tokens::map(ctx, tx, call); }
        if sel == swap_tokens_for_exact_tokens::SELECTOR { return swap_tokens_for_exact_tokens::map(ctx, tx, call); }
        if sel == swap_exact_eth_for_tokens::SELECTOR    { return swap_exact_eth_for_tokens::map(ctx, tx, call); }
        if sel == swap_tokens_for_exact_eth::SELECTOR    { return swap_tokens_for_exact_eth::map(ctx, tx, call); }
        if sel == swap_exact_tokens_for_eth::SELECTOR    { return swap_exact_tokens_for_eth::map(ctx, tx, call); }
        if sel == swap_eth_for_exact_tokens::SELECTOR    { return swap_eth_for_exact_tokens::map(ctx, tx, call); }
        if sel == swap_exact_tokens_for_tokens_supporting_fee_on_transfer_tokens::SELECTOR {
            return swap_exact_tokens_for_tokens_supporting_fee_on_transfer_tokens::map(ctx, tx, call);
        }
    ...
    if to_lc == V3_ROUTER_LC {
        if sel == exact_input_single::SELECTOR  { return exact_input_single::map(ctx, tx, call); }
        if sel == exact_input::SELECTOR         { return exact_input::map(ctx, tx, call); }
        if sel == exact_output_single::SELECTOR { return exact_output_single::map(ctx, tx, call); }
        if sel == exact_output::SELECTOR        { return exact_output::map(ctx, tx, call); }
    }

    if to_lc == SR02_LC {
        if sel == sr02_in_single::SELECTOR  { return sr02_in_single::map(ctx, tx, call); }
        if sel == sr02_in::SELECTOR         { return sr02_in::map(ctx, tx, call); }
        if sel == sr02_out_single::SELECTOR { return sr02_out_single::map(ctx, tx, call); }
        if sel == sr02_out::SELECTOR        { return sr02_out::map(ctx, tx, call); }
    }

    if is_universal_router(&to_lc) {
        if sel == ur_execute::SELECTOR_2ARGS || sel == ur_execute::SELECTOR_3ARGS {
