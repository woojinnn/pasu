...
8. **증거 ledger 없으면 완료가 아니다.** `crates/integration-tests/ONBOARDING_EVIDENCE_TEMPLATE.md` 를 `crates/integration-tests/onboarding/<protocol>/evidence.md` 로 복사해 채운다. P0 Claude/sub-agent, P2 Etherscan, P2 Dune 행이 `done` 또는 구체적 `blocked` 가 아니면 해당 phase 를 완료로 말하지 않는다. 사용자가 "했냐?"라고 물었을 때 "안 했습니다"가 아니라, ledger 의 명령·결과·카운트·blocker 로 답해야 한다.
...
| P4 final | exact changed files, gate outputs, remaining WARNs, explicit deferred selectors/actions with reason |
