...
# Pasu — 신규 프로토콜 온보딩 & V3 디코드 테스트 매뉴얼
...
Pasu = EVM 권한 위임을 **서명 직전**에 정적 분석하는 브라우저 익스텐션. raw Tx(calldata) 를 받아 **어떤 어댑터(manifest)를 쓸지 정하고 → 디코드 → 매핑 → `ActionBody[]`** 로 정규화한다. 이 매뉴얼이 다루는 것은 그 **디코드 경로(raw Tx → ActionBody[])의 정확성**이다.
...
4. **권한 grant 는 Unknown 금지** — `approve/permit/authorize/delegate/operator/allow` 류는 맞는 Action 이 없으면 Tier 3 를 추가한다. Pasu 핵심 surface 라서 "표현 안 됨"을 이유로 skip 하지 않는다.
...
| P2 real-tx Dune | MCP/API availability, usage baseline, query id/SQL summary with partition WHERE, rows returned, executionCostCredits or usage delta, selected tx hashes or explicit blocker; for pool-heavy/factory protocols, Dune selector/address stats over candidate universe when Etherscan cannot cover it; **SCOPE ORACLE coverage-share — volume-weighted protocol-level (Σ covered / Σ all top-level tx across every entry, H2) with wrapper/router selectors counted by child resolution-rate not manifest-presence (H3, §9.11)** |
...
4. **⚠️ permission-primitive red-flag (반드시 통과)** — `authorize | approve | permit | delegate | setOperator | setApprovalForAll | setAuthorization` 패턴 함수는 **무조건 COVER** (Tier 3 escalate 필요해도). Pasu 의 존재 이유(권한 위임 분석)라서 EXCLUDE/Unknown/skip **금지**. on-chain calldata 와 off-chain EIP-712(`*WithSig`/typed-data) **양쪽** 점검 — 둘 다 권한 grant 다. 매핑할 ActionBody 가 없으면 §4a 에서 Tier 3 신규 action 추가(Unknown 으로 떨구지 않음).
...
> ⚠️ Unknown 은 *진짜 opaque/admin* 호출용이지 **알려진 권한 grant 용이 아니다.** 권한 위임을 Unknown 으로 두면 분석기가 "전권 위임"을 opaque calldata 로만 보여줘 **과소경고** — Pasu 의 핵심 실패. 권한 grant 는 가치가 항상 높아 extension guide rule 3 의 'value<cost→Unknown' 을 override → Tier 3 escalate (§3 red-flag).
...
Pasu 은 사용자가 **서명 직전 intent 를 이해**하게 하는 도구다. 그러므로 P1 의 마지막 질문은 "디코드가 됐나"가 아니라 **"디코드된 필드가 사용자에게 그 자체로 읽히나? 안 읽히면 무엇으로 환산해 보여줘야 하나?"** 다. 이 단계를 건너뛰면 모든 게이트(check:surface / check:manifest / corpus / workspace)가 green 인데도 **user-illegible** 한 어댑터가 착지한다 — Lido 1차 온보딩이 정확히 그랬다(§9.9). decode-faithful("raw 필드만, live_inputs 비움")은 set_authorization 같은 **이미 읽히는** action 에만 맞다; **모든** action 의 기본값이 아니다.
...
### 8.4 정직한 한계
- **semantic 층은 객관 오라클이 없다.** "path[0]이 진짜 token-at-risk 인가", "domain=amm 인가" 는 Pasu 의미모델이 정의 — 체인이 안 알려줌. provenance(값 출처)·B projection(2nd-opinion)까지가 자동 검증의 한계. 필드 **역할** 정합성은 사람(또는 미래의 시뮬레이션 trace)이 보증.
...
§9 1차는 supply/withdraw/borrow/repay **4만** 보고 끝냈는데, 사용자가 "Morpho 함수가 더 많을 텐데?" 지적. 재triage(§3 보강된 P0)로 **user surface = 8** 확정 — 그중 `setAuthorization`(권한 위임)이 Pasu 핵심인데 1차에서 uncovered 로 버려졌었다.
...
**교훈**: 보강된 §3 surface-completeness gate + permission red-flag(Part A)가 이 누락을 1차에 잡았을 것. 권한 grant 는 Unknown 이 아니라 Tier 3(§4a override). Pasu 가장 핵심 함수가 manifest-only 가 아니라 schema 확장을 요구했다 — "쉬운 4개"만 집으면 정확히 그걸 놓친다.
...
### 9.11 SCOPE ORACLE — "all-green 인데 scope 가 틀렸다" (§9.4 의 scope-level 버전, 2026-06-02; 2026-06 balancer 2nd dogfood 로 H1/H2/H3 보강)
...
**2nd dogfood (Balancer 깊은 재온보딩, 2026-06).** 같은 실패가 재발했다 — 이번엔 *체인*이 아니라 **wrapper child + 거래량 가중**에서. Balancer 를 P0→P4 관통해 **전 게이트 green**(surface·universe·corpus 21/21·evidence 70/70) + 4 commit 까지 갔는데 프로토콜-레벨 실커버는 **~14.3%**(229/1600 mainnet 7d direct)뿐이었다. 두 맹점: (1) **거래량 가중 부재** — V2 Vault 를 깊게 파 98.9%(185/187) 커버했으나 그건 *작은* surface였고, 거래량 지배 entry 인 **V3 Router-v2(1262 tx, V2 의 6.7배)는 3.5%**만 잡았다. per-contract 로 보면 "V2 거의 다 커버"지만 프로토콜-레벨 거래량으로 보면 지배 surface 를 놓침. (2) **wrapper selector-presence 오산** — V3 의 91.7% 를 차지하는 `permitBatchAndCall` 에 `multicall_recurse` manifest 가 있어 "커버"로 셌으나, 그 child 의 **95% 가 deferred proportional/unbalanced liquidity**(pool-token list 가 calldata 에 없어 미배선) → **effective 4.9%**. manifest 존재를 coverage 로 센 게 핵심 오류. → H2(거래량 가중)·H3(wrapper=child resolution rate)·H1(이 둘을 *결정 전* 측정)로 SCOPE ORACLE 을 강화(아래 6~8).

**처치 = SCOPE ORACLE** (ONBOARDING_PROMPT `[SCOPE CONTRACT]` + 본 매뉴얼 P2 + evidence `Scope Classification`/`P2 Real-Tx` 의 SCOPE ORACLE row):
...
6. **(H1) 측정을 *결정 전* 입력으로** — coverage 측정을 P2(빌드 후)에만 두지 말 것. cover/defer 경계 *결정 전*(P0직후·P1전)에 cross-entry 거래량 분포 + cover 후보 wrapper 의 child resolution rate 를 실측해 경계의 사전입력으로 쓴다. P2 는 그 결정을 *검증*하는 자리. (morpho·balancer 둘 다 측정을 결정 후로 미뤄 틀린 scope 를 지었다.)
7. **(H2) coverage-share = 프로토콜-레벨 거래량 가중** — Σ covered top-level tx / Σ all top-level tx 를 **전 user-facing entry 합산**으로. 한 entry 의 per-contract selector-share 만 보면 거래량 지배 entry 를 놓친다(balancer: V2 Vault 98.9% 만 보고 "대부분 커버"는 거짓 — V3 Router-v2 가 거래량 6.7배·3.5%).
8. **(H3) wrapper-effective = child resolution rate** — wrapper/router selector(`multicall_recurse`·`opcode_stream_dispatch`·`tagged_dispatch`·`permitBatchAndCall` 류)는 **manifest 존재 = covered 가 아니다.** effective coverage = 실거래 child 중 covered child 비율. selector-presence 로 세면 child 전부 deferred 여도 "covered" 과대평가(balancer permitBatchAndCall: V3 selector 91.7% 인데 child 95% deferred → effective 4.9%). **정적 게이트(`check:surface` 등)는 selector-presence 만 보므로 이걸 못 잡는다** — SCOPE ORACLE 실거래 측정이 유일한 책임자.
