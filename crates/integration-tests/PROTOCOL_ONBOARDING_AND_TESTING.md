...
[입력: 새 프로토콜]
   ...
   │
   P4 LAND ── build-index → build-index vitest → check:manifest(emit.body shape) → check:surface → check:universe(pool/factory/vault-heavy) → coverage/fuzz/corpus → cargo test --workspace 0 fail → fmt/clippy → commit
```
