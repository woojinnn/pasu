...
최상위 12 variant (`action/mod.rs` — **작성 전 `grep -n "pub enum ActionBody"` 로 카운트 재확인**, 늘어난다):
`Token` · `Amm` · `Lending` · `Airdrop` · `Launchpad` · `Perp` · `LiquidStaking` · `Permission` · `Restaking` · `Staking` · `Multicall { actions: Vec<ActionBody> }` · `Unknown { target, chain, calldata, value }`
...
- L1 Envelope(ok 필드) / L2 TypedRoundTrip(`Vec<simulation_reducer::action::Action>` 역직렬화) / L3 Domain(`VALID_DOMAINS` 12종) / L4 ErrorClass.
