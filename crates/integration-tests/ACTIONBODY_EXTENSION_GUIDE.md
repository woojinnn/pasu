...
> `ActionBody` 를 확장하는 정규 절차. 1차 출처 = repo 코드 실측. (도메인 수는 늘어난다 — 현재 11: Token/Amm/Lending/Airdrop/Launchpad/Perp/LiquidStaking/Permission/Staking + Multicall/Unknown. `grep -n "pub enum ActionBody" action/mod.rs` 재확인.)
