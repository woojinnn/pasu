...
// `ClaimUnstakeAction.ticket` (required on the struct) has no counterpart in
// `policy-schema/actions/staking/claim_unstake.cedarschema`. Cedar rejects
...
mod tests {
    ...
    fn claim_unstake(recipient: crate::action::Address) -> ClaimUnstakeAction {
        ClaimUnstakeAction {
    ...
    #[test]
    fn claim_unstake_action_lowers_minimal_context() {
        let from = address("0x1111111111111111111111111111111111111111");
        let request = policy_request(
            &envelope(Action::ClaimUnstake(claim_unstake(from.clone()))),
            &from,
        );

        assert!(request.action.contains("claim_unstake"));
        assert_eq!(
            request
                .context
                .get("tokenOut")
    ...
    #[test]
    fn claim_unstake_action_lowers_full_context() {
        let from = address("0x1111111111111111111111111111111111111111");
        let mut action = claim_unstake(from.clone());
        action.amount_out = Some(amount(AmountKind::Exact, "999000000000000000"));

        let request = policy_request(&envelope(Action::ClaimUnstake(action)), &from);
