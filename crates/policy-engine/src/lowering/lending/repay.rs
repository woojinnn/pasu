...
use crate::context_keys::{AMOUNT, ASSET, VALIDITY_DELTA_SEC};
...
const REPAY_KIND: &str = "repayKind";
...
fn context(action: &RepayAction, ctx: &LoweringCtx<'_>) -> Result<Value, LoweringError> {
    ...
    context.insert(
        REPAY_KIND.into(),
        Value::from(repay_kind_str(&action.repay_kind)),
    );
...
const fn repay_kind_str(kind: &RepayKind) -> &'static str {
    match kind {
